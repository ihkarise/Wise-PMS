...
| Allowed access | For each role, a route/action **within** its permission set dispatches to the real handler. |
| Denied access | For each role, a route **outside** its permission set renders the fail-closed denial (not the handler), under a `MagicMock` page like `test_router.py`. |
...
- The new RBAC tables and indexes change the `TABLES:` and `INDEXES:` lines
  **once, intentionally**. Current golden lines:
  ...
  - `INDEXES: idx_attach_patient,…,idx_visit_patient`
  After Sprint 5 they additionally contain `permissions,role_permissions,
  roles,user_roles` (alphabetical position) and the new RBAC indexes.
